...
- `data`: the input data, a 3D array of shape (p, v, h), where p is the number of oblique planes (camera frames), v is the number of vertical camera pixels, and h is the number of horizontal camera pixels. (note that we are using numpy-standard C-order indexing: the last axis represents the most rapidly changing memory location)
