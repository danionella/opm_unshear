...
//
extern "C" __global__ void unshear_kernel(float * y, const int * y_sh, const float * x, const int *x_sh, const int sub_j, const int sup_i, const float slope, const float fill_value) {
    ...
    width_i = fmaxf(1.0f/(float)sup_i, 1.0f/slope);
        ...
        for (int j = blockIdx.y * blockDim.y + threadIdx.y; j < y_sh[1]; j += blockDim.y * gridDim.y) {
            for (int k = blockIdx.z * blockDim.z + threadIdx.z; k < y_sh[2]; k += blockDim.z * gridDim.z) {
                ...
                for (int jj = -slope_int; jj < slope_int+1; jj++){
                    s = j*sub_j+jj;
                    r_float = (float)i/(float)sup_i+(s-x_sh[1]/2)/slope;
...
def unshear(x, sub_j, sup_i, slope, out=None, tpb=[8, 8, 8], fill_value=0.0):
    ...
    Args:
        ...
        slope (float): shear slope in (px along dim 1)/(px along dim 0)
    ...
    if out is None:
        out = cp.zeros((x.shape[0] * sup_i, x.shape[1] // sub_j, x.shape[2]), dtype=x.dtype)
